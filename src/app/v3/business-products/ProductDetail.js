...
const pvModels = [
  ['P16V', '16', '15'], ['P21V', '21', '23'], ['P31V', '31', '23'],
  ['P40V', '40', '37'], ['P70V', '70', '63'], ['P100V', '100', '91'], ['P130V', '130', '112'],
];
...
const phCodeItems = [
  ...
  ['9', '펌프 제어방식', 'CH 압력보상 · CGH 원격압력보상 · CVH 로드센싱', 'TL/TH 토오크 리미트 · EDHS 전기 다이렉트 제어'],
  ['10', '최대 이론용적 조정기능', '무기호 : 없음', 'D : 있음'],
  ['11', '펌프 제어밸브 디자인 번호', '제어밸브 설계 버전을 나타내는 번호', ''],
  ['12', '관리기호', 'S38 : TL / TH형에 적용', '무기호 : 그 외'],
];

const tableClass = 'w-full min-w-[680px] border-collapse text-center text-[13px] text-[#344750]';
const thClass = 'border border-[#cfdade] bg-[#eef6f8] px-3 py-3 font-extrabold leading-[1.35] text-[#33505e]';
...
function SectionTitle({ eyebrow, title, description }) {
  return <header className="mb-8 grid grid-cols-[220px_1fr] gap-8 max-[760px]:grid-cols-1 max-[760px]:gap-3">
    <p className="m-0 text-[10px] font-extrabold tracking-[.14em] text-[#129fbe]">{eyebrow}</p>
    <div><h3 className="m-0 text-[clamp(27px,3.2vw,40px)] leading-[1.18] tracking-[-.07em]">{title}</h3>{description && <p className="mb-0 mt-4 max-w-[620px] break-keep text-sm leading-[1.75] text-[#6e7c83]">{description}</p>}</div>
  </header>;
}
...
function PvTechnicalDetails() {
  return <section className="col-span-full mt-20 border-t border-[#cbd6da] pt-16 max-[760px]:mt-14 max-[760px]:pt-11">
    ...
    <div className="grid grid-cols-12 gap-5">
      ...
      <TechnicalFigure className="col-span-8 max-[760px]:col-span-12" src="/product-details/pv-section.png" alt="P**V 시리즈 피스톤 펌프 단면도" label="INTERNAL STRUCTURE / SECTION VIEW" />
      <div className="col-span-8 flex flex-col justify-center bg-[#123a52] px-[clamp(28px,5vw,65px)] py-[clamp(36px,5vw,58px)] text-white max-[760px]:col-span-12">
        <p className="m-0 text-[10px] font-extrabold tracking-[.14em] text-[#7fd5e7]">PRODUCT OVERVIEW</p>
        <h4 className="mb-4 mt-3 text-[25px] tracking-[-.055em]">정밀 제어와 저소음을 함께 고려한 설계</h4>
        <p className="m-0 break-keep text-[15px] leading-[1.9] text-[#d3e3e9]">전기 다이렉트 제어를 시작으로 압력보상 제어, 로드센싱 제어 등 응답성과 안정성이 뛰어난 다양한 기능을 갖춘 피스톤 펌프입니다. 저소음·고성능·고신뢰성을 바탕으로 복잡한 시스템과 더블 펌프 구성에 대응하며, 장비의 에너지 절감과 고속화 요구에도 적용할 수 있습니다.</p>
      </div>
      <TechnicalFigure className="col-span-4 max-[760px]:col-span-12" src="/product-details/pv-symbol.png" alt="P**V 시리즈 유압 심벌" label="HYDRAULIC SYMBOL" />
    </div>
    <PvSpecification />
  </section>;
}
    ...
    <div className="grid grid-cols-[1.2fr_.8fr] gap-7 max-[900px]:grid-cols-1">
      <div className="overflow-x-auto">
        <table className={tableClass}>
          ...
          <thead><tr><th className={thClass} scope="col">형식</th><th className={thClass} scope="col">최대 이론용적<br /><small>cm³/rev</small></th><th className={thClass} scope="col">최고 사용압력<br /><small>MPa</small></th><th className={thClass} scope="col">최고 회전수<br /><small>min⁻¹</small></th><th className={thClass} scope="col">최저 회전수<br /><small>min⁻¹</small></th><th className={thClass} scope="col">무게<br /><small>kg</small></th></tr></thead>
          <tbody>{pvModels.map(([model, displacement, weight], index) => <tr key={model}><th className={`${tdClass} bg-white text-[#1163a2]`} scope="row">{model}</th><td className={tdClass}>{displacement}</td>{index === 0 && <><td className={tdClass} rowSpan={pvModels.length}>21</td><td className={tdClass} rowSpan={pvModels.length}>1,800</td><td className={tdClass} rowSpan={pvModels.length}>600</td></>}<td className={tdClass}>{weight}</td></tr>)}</tbody>
        </table>
        <ul className="mb-0 mt-4 grid gap-2 pl-5 text-xs leading-[1.65] text-[#6d7b82]"><li>무게는 C형(압력보상제어) 밸브의 수치입니다.</li><li>물·글리콜계 작동유를 사용하는 경우에는 적용 사양을 별도로 문의해 주십시오.</li></ul>
      </div>
      <div className="overflow-x-auto">
        <h5 className="mb-4 mt-0 text-sm tracking-[-.03em] text-[#40545f]">내장형 정용량형 펌프의 사양</h5>
        <table className={`${tableClass} min-w-[430px]`}><caption className="sr-only">P**V 내장형 정용량형 펌프 사양</caption><thead><tr><th className={thClass} scope="col">용량기호</th><th className={thClass} scope="col">이론용적<br /><small>cm³/rev</small></th><th className={thClass} scope="col">최고 사용압력<br /><small>MPa</small></th></tr></thead><tbody>{[['2', '6.3'], ['3', '9.4'], ['4', '12.5'], ['5', '15.6']].map(([code, displacement], index) => <tr key={code}><th className={`${tdClass} bg-white text-[#1163a2]`} scope="row">{code}</th><td className={tdClass}>{displacement}</td>{index === 0 && <td className={tdClass} rowSpan="4">16</td>}</tr>)}</tbody></table>
      </div>
    </div>
  </section>;
}
...
function SpecHeader() {
  return <div className="mb-6 flex items-end justify-between gap-5 border-b-[3px] border-[#129fbe] pb-4 max-[760px]:items-start max-[760px]:flex-col"><div><p className="m-0 text-[10px] font-extrabold tracking-[.14em] text-[#129fbe]">STANDARD SPECIFICATION</p><h4 className="mb-0 mt-2 text-[28px] tracking-[-.06em]">사양</h4></div><span className="text-xs text-[#718087]">단위: cm³/rev · MPa · min⁻¹ · kg</span></div>;
}
...
function PhTechnicalDetails() {
  return <section className="col-span-full mt-20 border-t border-[#cbd6da] pt-16 max-[760px]:mt-14 max-[760px]:pt-11">
    ...
    <div className="grid grid-cols-12 gap-5">
      ...
      <TechnicalFigure className="col-span-8 max-[760px]:col-span-12" src="/product-details/ph-section.png" alt="PH 시리즈 피스톤 펌프 단면도" label="INTERNAL STRUCTURE / SECTION VIEW" />
      <div className="col-span-8 flex flex-col justify-center bg-[#123a52] px-[clamp(28px,5vw,65px)] py-[clamp(36px,5vw,58px)] text-white max-[760px]:col-span-12"><p className="m-0 text-[10px] font-extrabold tracking-[.14em] text-[#7fd5e7]">HIGH PRESSURE SERIES</p><h4 className="mb-4 mt-3 text-[25px] tracking-[-.055em]">저소음과 고압 운전을 위한 PH 시리즈</h4><p className="m-0 break-keep text-[15px] leading-[1.9] text-[#d3e3e9]">PH80·PH100·PH130으로 구성되며 정격 28 MPa, 간헐 30 MPa의 고압 회로에 대응합니다. 압력보상, 원격압력보상, 로드센싱, 토오크 리미트와 전기 다이렉트 제어 등 적용 조건에 맞춘 제어 방식을 선택할 수 있습니다.</p></div>
      <TechnicalFigure className="col-span-4 max-[760px]:col-span-12" src="/product-details/ph-symbol.png" alt="PH 시리즈 유압 심벌" label="HYDRAULIC SYMBOL" />
    </div>
    <PhModelCode />
    <PhSpecification />
  </section>;
...
function ProductTechnicalDetails({ pump }) {
  return pump.slug === 'pv-series' ? <PvTechnicalDetails /> : <PhTechnicalDetails />;
}
