"""Refresh only the SQP/SQPS single-vane catalog preview.

Run: python scripts/render-sqp-catalog-preview.py
...
HTML = ROOT / "public/sqp-sqps-single.html"
...
def main():
    ...
    output = CATALOG / "single-preview"
    output.mkdir(parents=True, exist_ok=True)
    figures = []
    with tempfile.TemporaryDirectory(prefix="sqp-catalog-") as temporary:
        subprocess.run(
            [renderer, "-png", "-scale-to-x", "1600", "-scale-to-y", "-1",
             str(CATALOG / "sqp-sqps-single.pdf"), str(Path(temporary) / "page")],
            check=True,
        )
        ...
        for number, path in enumerate(pages, 1):
            ...
            url = f"./sqp-pump-detail/catalogs/single-preview/{stem}"
            figures.append(
                ...
                f'alt="SQP/SQPS 1연 카탈로그 {number}페이지" loading="lazy" decoding="async">\n'
